...
| 包名 | 版本 | 用途 |
|------|------|------|
| `next` | `15.5.21` | App Router、Route Handler、SSR/RSC |
| `react` | `^18` | UI 框架 |
| `react-dom` | `^18` | DOM 渲染 |
...
| `eslint` | `^8` | 代码规范 |
| `eslint-config-next` | `15.5.21` | Next.js ESLint 规则集 |
| `autoprefixer` | `^10` | CSS 自动前缀 |
| `postcss` | `^8` | CSS 处理管道 |

## 4. Phase 4 Intelligence Enhancement

Phase 4 在既有 Router → Service → Domain → Database 分层上新增四个独立 intelligence 模块：

| 模块 | 文件 | 职责 |
|---|---|---|
| Cognitive Model | `backend/src/intelligence/cognitive_model.py` | 认知画像与遗忘曲线 |
| Memory System | `backend/src/intelligence/memory_system.py` | 三层记忆与相关性检索 |
| Adaptive Planner | `backend/src/intelligence/adaptive_planner.py` | 失败预测与 Proposal 生成 |
| Knowledge Graph | `backend/src/intelligence/knowledge_graph.py` | Concept/Resource 图与知识缺口 |
| Evaluation | `backend/src/intelligence/evaluation.py` | 版本化 benchmark 和指标持久化 |

DecisionContext 现在同时读取 Profile、Cognitive Profile、Active Pattern、三层 Memory、Knowledge Gap、Recent Events 与 Goal Context。所有写业务动作仍通过 DecisionProposal → User Review → Apply Gateway。

## 5. Phase 4.5 Production Hardening

- 应用时间统一由 `backend/src/core/time.py::utc_now()` 提供；当前数据库继续存储 naive UTC。
- 业务 Alembic 只管理 PlanPilot metadata；LangGraph `checkpoint_*` 表为 runtime 外部管理对象。
- migration `y8z9a0b1c2d3` 对齐业务列 NOT NULL 约束，`alembic check` 必须保持无 drift。
- Failure Prediction、Cognitive、Retention 与 Knowledge Gap 继续作为可解释 heuristic baseline；进入 ML 前必须完成版本化 feature/label、离线校准和 shadow 验证。
