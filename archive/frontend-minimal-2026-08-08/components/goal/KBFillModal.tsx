...
export default function KBFillModal({ open, kbId, goalId, kbName, onDone }: KBFillModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
      <div role="dialog" aria-modal="true" aria-label="向知识库添加内容" className="journal-dialog bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[85vh]">
        {/* 头部 */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div>
            ...
            <button
              ...
              className="w-full border border-dashed border-gray-300 rounded-lg py-3 text-xs text-gray-500 hover:border-accent hover:text-accent transition"
            >
              点击选择文件
            </button>
            <input
        ...
        <div className="px-5 py-4 border-t border-gray-100 flex gap-2">
          ...
          <button
            ...
            onClick={handleDone}
            disabled={isUploading}
            className="flex-[2] py-2.5 bg-accent text-white rounded-lg text-sm font-medium hover:bg-accent-dark disabled:opacity-50 transition flex items-center justify-center gap-2"
          >
            {isUploading ? (
