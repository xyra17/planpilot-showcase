...
The search script lives inside this skill's own directory, not the project directory. Resolve `<skill-dir>` to the directory containing this `SKILL.md`, then invoke the script by its full path — do not assume a particular working directory:
