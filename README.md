...
![今日行动：已登录 admin 的真实学习日](assets/planpilot-today-admin-annotated.png)
...
![PlanPilot 管理后台的用户价值验证](assets/planpilot-admin-product.png)

管理后台不是学习者工作台的数据汇总，而是内部产品验证与运行控制面。它将“是否创造学习价值”“AI 行动是否安全执行”“版本是否可以放量”拆开观察。

图中的 86 位授权用户及各项转化率是合成运营样本，用来验证看板结构、指标口径和早期样本不足时的表达方式，并非真实生产用户或商业成绩。每项指标同时展示分子、分母、观察窗口和预设线；样本不足时明确标记“证据还在积累”，避免把 mock 数据包装成业务结论。
