...
以下截图均来自已登录的 admin 学习空间，不再使用访客演示数据。空间内持久化了 3 个长期目标、15 份真实上传资料、19 篇笔记、4 组 Pilo 对话，以及历史、今日和宏观计划任务。用户和学习行为是为产品验证构造的长期样本；资料文件来自公开高校、考试机构与开源项目。
...
1. 今日行动不是任务标题列表，每项任务带目标、预计时长和执行说明。
2. 完成状态与实际投入分开记录，避免把一次勾选等同于学习效果。
3. 课程占用会影响可用时间，但不会被统计为目标学习负荷。
...
![Pilo 学习伙伴与编号说明](assets/planpilot-coach-admin-annotated.png)
...
### 6. 长期理解：用户能校正 AI，而不是被画像定义

![个性化与学习偏好管理](assets/planpilot-memory-admin.png)

系统观察被明确标注为暂定判断。用户可以查看依据、确认、修正适用范围、暂停或永久遗忘；只有确认或修正后的观察才应稳定影响后续建议。
