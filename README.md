...
PlanPilot 让 AI 处理需要理解语义的部分：目标、资料、当前水平、学习行为和计划偏差。日期计算、可学习天数、每日容量、日期分配、版本校验和真实写入交给确定性规则。这样，AI 可以帮助用户形成方案，系统也能对时间和动作做出稳定、可解释的约束。
...
## 管理控制台：把 AI 产品变成可运营系统

学习者端负责目标、行动和恢复，内部管理控制台负责判断产品有没有创造价值，以及 AI 功能能否安全放量。两套界面和权限彼此隔离，普通用户看不到后台入口，管理员直接访问地址也要经过权限校验。

控制台把三类问题拆开呈现：

- **产品验证：** 观察 10 分钟建好目标、24 小时首次行动、第 4 周学习闭环和中断后 72 小时恢复，并同时展示分子、分母、观察窗口和预设线；
- **行动验证：** 查看一次 AI 请求从需求理解、预览、确认到执行完成的漏斗，同时管理复核队列与紧急停用；
- **发布与运行：** 用离线评估、灰度发布、暂停／回滚、模型网关和调用审计控制上线风险，并从单次 Trace 回溯异常节点。

![PlanPilot 管理控制台：用户价值验证](assets/planpilot-admin-product.png)

上图使用合成运营样本，展示的是看板结构和判断口径。页面会在样本不足时明确提示“证据还在积累”，避免把早期波动写成业务结论。这个设计也让用户结果、AI 输出质量和安全事件可以分别归因，方便产品、模型和工程团队采取不同动作。
