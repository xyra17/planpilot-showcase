...
import { Home, Target, BookOpen, FileText, Settings, ChevronLeft, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="flex h-screen bg-white">
      <aside className={cn(
        ...
        {/* Logo + 收缩按钮 */}
        <div className={cn(
          "flex items-center border-b border-gray-100 h-[60px] flex-shrink-0",
          collapsed ? "justify-center px-2" : "justify-between px-5"
        )}>
          {!collapsed && (
            <>
              <span className="text-lg font-bold text-gray-900 whitespace-nowrap">PlanPilot</span>
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 transition flex-shrink-0"
          >
        ...
        <div className="px-2 py-3 border-t border-gray-100 flex-shrink-0">
          ...
          >
            <div className="w-7 h-7 rounded-full text-white flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ background: "var(--accent)" }}>
              {avatarText}
            </div>
            {!collapsed && (
              <>
