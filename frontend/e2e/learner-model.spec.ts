...
const context = { profile: null, cognitive_profile: { procrastination_score: 0.9, persistence_score: 0.2, challenge_tolerance: 0.1, feedback_acceptance: 0.3 }, memories: { short_term: [], episodic: [], semantic: [] }, knowledge_gaps: [], recent_events: [], goal_context: null, data_quality: { profile_event_count: 18, profile_scope: "user", pattern_count: 2, memory_count: 0, cognitive_confidence: 0.62, knowledge_gap_count: 0, low_confidence_fields: [], level: "medium" }, active_patterns: patterns };
...
async function openMemory(page: Page, options: { actionBodies?: Array<Record<string, unknown>>; failFirstCorrection?: boolean; undoCalls?: string[] } = {}) {
...
test("主导航隐藏学习记忆并保留情境深链", async ({ page }) => {
  ...
  await expect(target.getByText("当前精确置信度为 82%", { exact: false })).toBeVisible();
});

test("普通用户只看到两组可纠正观察且不暴露技术画像", async ({ page }) => {
  await openMemory(page);
...
test("375px 窄屏无横向滚动且操作达到 44px", async ({ page }) => {
