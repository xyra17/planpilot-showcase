...
async def checkin(
    ...
    if body.mode == "daily":
        ...
        execution_completed = sum(1 for t in body.tasks if t.status == "completed")
        execution_partial = sum(1 for t in body.tasks if t.status == "partial")
        completion_rate = (
            (execution_completed + execution_partial * 0.5) / total if total else 0.0
        )
        ...
        mastery_rate = ((good_count + basic_count * 0.5) / total) if total else 0.0
        ...
        mastery_rate = 0.0
        stats_data = {"total_tasks": total, "completion_rate": completion_rate, "mastery_rate": mastery_rate, "completed": completed, "partial": partial, "skipped": skipped, "estimated_mins": estimated_mins_total}
    ...
    replan = total > 0 and should_suggest_replan(
    ...
    if today_records:
        ...
        record.replan_triggered = replan
